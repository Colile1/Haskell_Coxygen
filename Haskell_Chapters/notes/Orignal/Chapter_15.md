# Haskell Chapter 15: Exception and Error Handling
...
Sadly, the real world is messy, and unexpected things can–and do–happen. When something unexpected happens, that's when we get an Exception. For example, here are a few (but not all) of the possible Exceptions that the program could throw at us:
...
Currently, all these Exceptions cause our program to crash. But, of course, a robust program should not collapse if something unexpected happens. Luckily, we have a couple of ways to handle these Exceptions.
...
If we write something different than any of those three constructors, our program halts, and subsequently, the car crashes with us inside.
And the other one is that the error provided in the case it fails doesn't provide much actionable information:
*** Exception: Prelude.read: no parse
...
Note: We have to import Control.Exception, which is the standard library module that contains code related to exceptions.
...
main = do
  ...
  case vel (readMaybe d) (readMaybe t) of
    ...
    Nothing -> do
      ...
      main
We do have a small issue now. velocity cannot take Ints directly anymore because readMaybe returns a Maybe. So we have to modify velocity like this:

vel :: Maybe Int -> Maybe Int -> Maybe Int
vel (Just dist) (Just time) = dist `safeDiv` time
vel  _           _          = Nothing
We pattern match for the cases when both are parsed correctly and pass the Ints to the safeDiv function. We ignore all other cases and return Nothing.
...
We will stick to Strings for today's examples. To keep it simple.
...
They are evident by the types, so you know when you're dealing with them | They are hidden from you, so you're unaware of how a function can fail by only looking at its type| | They are pure values, easy to predict | You can only catch them inside impure code, which makes your program harder to predict| | They are easy to use | They can be complicated to use | | Managing multiple levels of optional values may* add complexity to the code | The code complexity of the "happy path" stays the same, but there's added complexity elsewhere. | | They don't compose well with functions that take the unwrapped value. Which generates a loss in modularity | They don't affect composability | | Accidental strictness: It's not unavoidable, but it becomes easy to make our functions strict | Doesn't affect laziness and compiler optimizations | | They can't handle asynchronous Exceptions cause those happen outside the code | We can catch asynchronous Exceptions |

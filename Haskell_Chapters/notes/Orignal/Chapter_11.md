...
One is that, after performing the action, we get a VALUE of the specified type we can use in our code.
The other is that the action returns a value AFTER performing the IO action. We CAN NOT get that value without interacting with the real world.
And finally, we see a new type in the last action: The unit type.
The unit type
...
First, it performs the side effect of asking for our name in the standard output.
Then, it performs the side effect of waiting for us to type it and returning that as a value. But because we're using the >> operator, we ignore it.
And finally, it sends the last message.
